function [r2,glm,rPAC,cAMP] = cfc_est_glmstats(modulating_signal,modulated_signal, distr)
...
modulating_signal = (modulating_signal - mean(modulating_signal))/std(modulating_signal); 
   ...
   rPAC(ep)= sqrt([(glm.beta(1)^2)+(glm.beta(2)^2)]); 
   cAMP(ep)= glm.beta(1); 
end
